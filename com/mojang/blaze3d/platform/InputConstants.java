...
public class InputConstants {
   ...
   public static InputConstants.Key getKey(String s) {
      if (InputConstants.Key.NAME_MAP.containsKey(s)) {
         return InputConstants.Key.NAME_MAP.get(s);
      } else {
         for(InputConstants.Type inputconstants_type : InputConstants.Type.values()) {
            if (s.startsWith(inputconstants_type.defaultPrefix)) {
               String s1 = s.substring(inputconstants_type.defaultPrefix.length() + 1);
               int i = Integer.parseInt(s1);
               if (inputconstants_type == InputConstants.Type.MOUSE) {
                  --i;
               }

               return inputconstants_type.getOrCreate(i);
            }
         }
   ...
   public static boolean isKeyDown(long i, int j) {
      return GLFW.glfwGetKey(i, j) == 1;
   }

   public static void setupKeyboardCallbacks(long i, GLFWKeyCallbackI glfwkeycallbacki, GLFWCharModsCallbackI glfwcharmodscallbacki) {
      GLFW.glfwSetKeyCallback(i, glfwkeycallbacki);
      GLFW.glfwSetCharModsCallback(i, glfwcharmodscallbacki);
   }

   public static void setupMouseCallbacks(long i, GLFWCursorPosCallbackI glfwcursorposcallbacki, GLFWMouseButtonCallbackI glfwmousebuttoncallbacki, GLFWScrollCallbackI glfwscrollcallbacki, GLFWDropCallbackI glfwdropcallbacki) {
      GLFW.glfwSetCursorPosCallback(i, glfwcursorposcallbacki);
      GLFW.glfwSetMouseButtonCallback(i, glfwmousebuttoncallbacki);
      GLFW.glfwSetScrollCallback(i, glfwscrollcallbacki);
      GLFW.glfwSetDropCallback(i, glfwdropcallbacki);
   }

   public static void grabOrReleaseMouse(long i, int j, double d0, double d1) {
      GLFW.glfwSetCursorPos(i, d0, d1);
      GLFW.glfwSetInputMode(i, 208897, j);
   }
   ...
   public static void updateRawMouseInput(long i, boolean flag) {
      if (isRawMouseInputSupported()) {
         GLFW.glfwSetInputMode(i, GLFW_RAW_MOUSE_MOTION, flag ? 1 : 0);
      }
   ...
   static {
      ...
      try {
         methodhandle = methodhandles_lookup.findStatic(GLFW.class, "glfwRawMouseMotionSupported", methodtype);
         MethodHandle methodhandle1 = methodhandles_lookup.findStaticGetter(GLFW.class, "GLFW_RAW_MOUSE_MOTION", Integer.TYPE);
   ...
   public static final class Key {
      ...
      Key(String s, InputConstants.Type inputconstants_type, int i) {
         this.name = s;
         this.type = inputconstants_type;
         this.value = i;
         this.displayName = new LazyLoadedValue<>(() -> inputconstants_type.displayTextSupplier.apply(i, s));
         NAME_MAP.put(s, this);
      }
      ...
      public boolean equals(Object object) {
         if (this == object) {
            ...
            InputConstants.Key inputconstants_key = (InputConstants.Key)object;
            return this.value == inputconstants_key.value && this.type == inputconstants_key.type;
         } else {
            return false;
         }
      }
   ...
   public static enum Type {
      KEYSYM("key.keyboard", (integer, s) -> {
         if ("key.keyboard.unknown".equals(s)) {
            return Component.translatable(s);
         } else {
            String s1 = GLFW.glfwGetKeyName(integer, -1);
            return s1 != null ? Component.literal(s1.toUpperCase(Locale.ROOT)) : Component.translatable(s);
         }
      }),
      SCANCODE("scancode", (integer, s) -> {
         String s1 = GLFW.glfwGetKeyName(-1, integer);
         return s1 != null ? Component.literal(s1) : Component.translatable(s);
      }),
      MOUSE("key.mouse", (integer, s) -> Language.getInstance().has(s) ? Component.translatable(s) : Component.translatable("key.mouse", integer + 1));
      ...
      private static void addKey(InputConstants.Type inputconstants_type, String s, int i) {
         InputConstants.Key inputconstants_key = new InputConstants.Key(s, inputconstants_type, i);
         inputconstants_type.map.put(i, inputconstants_key);
      }

      private Type(String s, BiFunction<Integer, String, Component> bifunction) {
         this.defaultPrefix = s;
         this.displayTextSupplier = bifunction;
      }

      public InputConstants.Key getOrCreate(int i) {
         return this.map.computeIfAbsent(i, (j) -> {
            int k = j;
            if (this == MOUSE) {
               k = j + 1;
            }

            String s = this.defaultPrefix + "." + k;
            return new InputConstants.Key(s, this, j);
         });
      }
