...
public class VanillaRegistries {
   ...
   private static void validateThatAllBiomeFeaturesHaveBiomeFilter(HolderLookup.Provider holderlookup_provider) {
      validateThatAllBiomeFeaturesHaveBiomeFilter(holderlookup_provider.lookupOrThrow(Registries.PLACED_FEATURE), holderlookup_provider.lookupOrThrow(Registries.BIOME));
   }
   ...
   public static void validateThatAllBiomeFeaturesHaveBiomeFilter(HolderGetter<PlacedFeature> holdergetter, HolderLookup<Biome> holderlookup) {
      holderlookup.listElements().forEach((holder_reference) -> {
         ResourceLocation resourcelocation = holder_reference.key().location();
         List<HolderSet<PlacedFeature>> list = holder_reference.value().getGenerationSettings().features();
         list.stream().flatMap(HolderSet::stream).forEach((holder) -> holder.unwrap().ifLeft((resourcekey) -> {
               Holder.Reference<PlacedFeature> holder_reference3 = holdergetter.getOrThrow(resourcekey);
               if (!validatePlacedFeature(holder_reference3.value())) {
               ...
               if (!validatePlacedFeature(placedfeature)) {
                  Util.logAndPauseIfInIde("Placed inline feature in biome " + holder_reference + " is missing BiomeFilter.biome()");
               }
   ...
   public static HolderLookup.Provider createLookup() {
      RegistryAccess.Frozen registryaccess_frozen = RegistryAccess.fromRegistryOfRegistries(BuiltInRegistries.REGISTRY);
      HolderLookup.Provider holderlookup_provider = BUILDER.build(registryaccess_frozen);
      validateThatAllBiomeFeaturesHaveBiomeFilter(holderlookup_provider);
