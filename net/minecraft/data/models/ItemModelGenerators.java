...
public class ItemModelGenerators {
   ...
   private void generateFlatItem(Item item, String s, ModelTemplate modeltemplate) {
      modeltemplate.create(ModelLocationUtils.getModelLocation(item, s), TextureMapping.layer0(TextureMapping.getItemTexture(item, s)), this.output);
   }

   private void generateFlatItem(Item item, Item item1, ModelTemplate modeltemplate) {
      modeltemplate.create(ModelLocationUtils.getModelLocation(item), TextureMapping.layer0(item1), this.output);
   }
   ...
   private void generateLayeredItem(ResourceLocation resourcelocation, ResourceLocation resourcelocation1, ResourceLocation resourcelocation2) {
      ModelTemplates.TWO_LAYERED_ITEM.create(resourcelocation, TextureMapping.layered(resourcelocation1, resourcelocation2), this.output);
   }

   private void generateLayeredItem(ResourceLocation resourcelocation, ResourceLocation resourcelocation1, ResourceLocation resourcelocation2, ResourceLocation resourcelocation3) {
      ModelTemplates.THREE_LAYERED_ITEM.create(resourcelocation, TextureMapping.layered(resourcelocation1, resourcelocation2, resourcelocation3), this.output);
   }

   private ResourceLocation getItemModelForTrimMaterial(ResourceLocation resourcelocation, String s) {
      return resourcelocation.withSuffix("_" + s + "_trim");
   }
   ...
   private JsonObject generateBaseArmorTrimTemplate(ResourceLocation resourcelocation, Map<TextureSlot, ResourceLocation> map, ArmorMaterial armormaterial) {
      ...
      for(ItemModelGenerators.TrimModelData itemmodelgenerators_trimmodeldata : GENERATED_TRIM_MODELS) {
         JsonObject jsonobject1 = new JsonObject();
         JsonObject jsonobject2 = new JsonObject();
         jsonobject2.addProperty(TRIM_TYPE_PREDICATE_ID.getPath(), itemmodelgenerators_trimmodeldata.itemModelIndex());
         jsonobject1.add("predicate", jsonobject2);
         jsonobject1.addProperty("model", this.getItemModelForTrimMaterial(resourcelocation, itemmodelgenerators_trimmodeldata.name(armormaterial)).toString());
   ...
   private void generateArmorTrims(ArmorItem armoritem) {
      ...
      if (armoritem.getMaterial() == ArmorMaterials.LEATHER) {
         ModelTemplates.TWO_LAYERED_ITEM.create(resourcelocation, TextureMapping.layered(resourcelocation1, resourcelocation2), this.output, (resourcelocation6, map1) -> this.generateBaseArmorTrimTemplate(resourcelocation6, map1, armoritem.getMaterial()));
      } else {
         ModelTemplates.FLAT_ITEM.create(resourcelocation, TextureMapping.layer0(resourcelocation1), this.output, (resourcelocation5, map) -> this.generateBaseArmorTrimTemplate(resourcelocation5, map, armoritem.getMaterial()));
      }

      for(ItemModelGenerators.TrimModelData itemmodelgenerators_trimmodeldata : GENERATED_TRIM_MODELS) {
         String s = itemmodelgenerators_trimmodeldata.name(armoritem.getMaterial());
