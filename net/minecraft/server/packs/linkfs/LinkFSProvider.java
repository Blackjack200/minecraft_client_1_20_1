...
class LinkFSProvider extends FileSystemProvider {
   ...
   public DirectoryStream<Path> newDirectoryStream(Path path, final DirectoryStream.Filter<? super Path> directorystream_filter) throws IOException {
      final PathContents.DirectoryContents pathcontents_directorycontents = toLinkPath(path).toAbsolutePath().getDirectoryContents();
      if (pathcontents_directorycontents == null) {
         throw new NotDirectoryException(path.toString());
      } else {
         return new DirectoryStream<Path>() {
            public Iterator<Path> iterator() {
               return pathcontents_directorycontents.children().values().stream().filter((linkfspath1) -> {
                  try {
                     return directorystream_filter.accept(linkfspath1);
   ...
   public void checkAccess(Path path, AccessMode... aaccessmode) throws IOException {
      if (aaccessmode.length == 0 && !toLinkPath(path).exists()) {
         ...
         AccessMode[] var3 = aaccessmode;
         int var4 = aaccessmode.length;
         int var5 = 0;
   ...
   public <V extends FileAttributeView> V getFileAttributeView(Path path, Class<V> oclass, LinkOption... alinkoption) {
      ...
      return (V)(oclass == BasicFileAttributeView.class ? linkfspath.getBasicAttributeView() : null);
   }
