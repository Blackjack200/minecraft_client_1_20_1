...
public class SilverfishModel<T extends Entity> extends HierarchicalModel<T> {
   ...
   public SilverfishModel(ModelPart modelpart) {
      this.root = modelpart;
      Arrays.setAll(this.bodyParts, (j) -> modelpart.getChild(getSegmentName(j)));
   ...
   public void setupAnim(T entity, float f, float f1, float f2, float f3, float f4) {
      for(int i = 0; i < this.bodyParts.length; ++i) {
         this.bodyParts[i].yRot = Mth.cos(f2 * 0.9F + (float)i * 0.15F * (float)Math.PI) * (float)Math.PI * 0.05F * (float)(1 + Math.abs(i - 2));
         this.bodyParts[i].x = Mth.sin(f2 * 0.9F + (float)i * 0.15F * (float)Math.PI) * (float)Math.PI * 0.2F * (float)Math.abs(i - 2);
      }
