...
public class PandaModel<T extends Panda> extends QuadrupedModel<T> {
   ...
   public void prepareMobModel(T panda, float f, float f1, float f2) {
      super.prepareMobModel(panda, f, f1, f2);
      this.sitAmount = panda.getSitAmount(f2);
      this.lieOnBackAmount = panda.getLieOnBackAmount(f2);
      this.rollAmount = panda.isBaby() ? 0.0F : panda.getRollAmount(f2);
   }

   public void setupAnim(T panda, float f, float f1, float f2, float f3, float f4) {
      super.setupAnim(panda, f, f1, f2, f3, f4);
      boolean flag = panda.getUnhappyCounter() > 0;
      boolean flag1 = panda.isSneezing();
      int i = panda.getSneezeCounter();
      boolean flag2 = panda.isEating();
      boolean flag3 = panda.isScared();
      if (flag) {
         this.head.yRot = 0.35F * Mth.sin(0.6F * f2);
         this.head.zRot = 0.35F * Mth.sin(0.6F * f2);
         this.rightFrontLeg.xRot = -0.75F * Mth.sin(0.3F * f2);
         this.leftFrontLeg.xRot = 0.75F * Mth.sin(0.3F * f2);
      } else {
         this.head.zRot = 0.0F;
      }
      ...
      if (flag1) {
         if (i < 15) {
            ...
            float f5 = (float)((i - 15) / 5);
            this.head.xRot = (-(float)Math.PI / 4F) + ((float)Math.PI / 4F) * f5;
         }
      }
      ...
      if (this.sitAmount > 0.0F) {
         ...
         if (flag2) {
            this.head.xRot = ((float)Math.PI / 2F) + 0.2F * Mth.sin(f2 * 0.6F);
            this.rightFrontLeg.xRot = -0.4F - 0.2F * Mth.sin(f2 * 0.6F);
            this.leftFrontLeg.xRot = -0.4F - 0.2F * Mth.sin(f2 * 0.6F);
         }
      ...
      if (this.lieOnBackAmount > 0.0F) {
         this.rightHindLeg.xRot = -0.6F * Mth.sin(f2 * 0.15F);
         this.leftHindLeg.xRot = 0.6F * Mth.sin(f2 * 0.15F);
         this.rightFrontLeg.xRot = 0.3F * Mth.sin(f2 * 0.25F);
         this.leftFrontLeg.xRot = -0.3F * Mth.sin(f2 * 0.25F);
      ...
      if (this.rollAmount > 0.0F) {
         ...
         this.rightHindLeg.xRot = -0.5F * Mth.sin(f2 * 0.5F);
         this.leftHindLeg.xRot = 0.5F * Mth.sin(f2 * 0.5F);
         this.rightFrontLeg.xRot = 0.5F * Mth.sin(f2 * 0.5F);
         this.leftFrontLeg.xRot = -0.5F * Mth.sin(f2 * 0.5F);
      }
