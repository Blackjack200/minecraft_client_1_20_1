...
public class EndermiteModel<T extends Entity> extends HierarchicalModel<T> {
   ...
   public void setupAnim(T entity, float f, float f1, float f2, float f3, float f4) {
      for(int i = 0; i < this.bodyParts.length; ++i) {
         this.bodyParts[i].yRot = Mth.cos(f2 * 0.9F + (float)i * 0.15F * (float)Math.PI) * (float)Math.PI * 0.01F * (float)(1 + Math.abs(i - 2));
         this.bodyParts[i].x = Mth.sin(f2 * 0.9F + (float)i * 0.15F * (float)Math.PI) * (float)Math.PI * 0.1F * (float)Math.abs(i - 2);
      }
