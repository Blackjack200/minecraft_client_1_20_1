...
public class TurtleModel<T extends Turtle> extends QuadrupedModel<T> {
   ...
   public void setupAnim(T turtle, float f, float f1, float f2, float f3, float f4) {
      super.setupAnim(turtle, f, f1, f2, f3, f4);
      this.rightHindLeg.xRot = Mth.cos(f * 0.6662F * 0.6F) * 0.5F * f1;
      this.leftHindLeg.xRot = Mth.cos(f * 0.6662F * 0.6F + (float)Math.PI) * 0.5F * f1;
      this.rightFrontLeg.zRot = Mth.cos(f * 0.6662F * 0.6F + (float)Math.PI) * 0.5F * f1;
      this.leftFrontLeg.zRot = Mth.cos(f * 0.6662F * 0.6F) * 0.5F * f1;
      ...
      if (!turtle.isInWater() && turtle.onGround()) {
         float f5 = turtle.isLayingEgg() ? 4.0F : 1.0F;
         float f6 = turtle.isLayingEgg() ? 2.0F : 1.0F;
         float f7 = 5.0F;
         this.rightFrontLeg.yRot = Mth.cos(f5 * f * 5.0F + (float)Math.PI) * 8.0F * f1 * f6;
         this.rightFrontLeg.zRot = 0.0F;
         this.leftFrontLeg.yRot = Mth.cos(f5 * f * 5.0F) * 8.0F * f1 * f6;
         this.leftFrontLeg.zRot = 0.0F;
         this.rightHindLeg.yRot = Mth.cos(f * 5.0F + (float)Math.PI) * 3.0F * f1;
         this.rightHindLeg.xRot = 0.0F;
         this.leftHindLeg.yRot = Mth.cos(f * 5.0F) * 3.0F * f1;
   ...
   public void renderToBuffer(PoseStack posestack, VertexConsumer vertexconsumer, int i, int j, float f, float f1, float f2, float f3) {
      ...
      super.renderToBuffer(posestack, vertexconsumer, i, j, f, f1, f2, f3);
      if (flag) {
         posestack.popPose();
      }
