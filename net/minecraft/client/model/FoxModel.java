...
public class FoxModel<T extends Fox> extends AgeableListModel<T> {
   ...
   public void prepareMobModel(T fox, float f, float f1, float f2) {
      ...
      this.head.yRot = 0.0F;
      this.head.zRot = fox.getHeadRollAngle(f2);
      ...
      if (fox.isCrouching()) {
         ...
         float f3 = fox.getCrouchAmount(f2);
         this.body.setPos(0.0F, 16.0F + fox.getCrouchAmount(f2), -6.0F);
         this.head.setPos(-1.0F, 16.5F + f3, -3.0F);
         this.head.yRot = 0.0F;
   ...
   public void setupAnim(T fox, float f, float f1, float f2, float f3, float f4) {
      ...
      if (fox.isSleeping()) {
         this.head.xRot = 0.0F;
         this.head.yRot = -2.0943952F;
         this.head.zRot = Mth.cos(f2 * 0.027F) / 22.0F;
      }

      if (fox.isCrouching()) {
         float f5 = Mth.cos(f2) * 0.01F;
         this.body.yRot = f5;
         this.rightHindLeg.zRot = f5;
         this.leftHindLeg.zRot = f5;
         this.rightFrontLeg.zRot = f5 / 2.0F;
         this.leftFrontLeg.zRot = f5 / 2.0F;
      }
