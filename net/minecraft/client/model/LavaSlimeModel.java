...
public class LavaSlimeModel<T extends Slime> extends HierarchicalModel<T> {
   ...
   public void setupAnim(T slime, float f, float f1, float f2, float f3, float f4) {
   }

   public void prepareMobModel(T slime, float f, float f1, float f2) {
      float f3 = Mth.lerp(f2, slime.oSquish, slime.squish);
      if (f3 < 0.0F) {
         f3 = 0.0F;
      }

      for(int i = 0; i < this.bodyCubes.length; ++i) {
         this.bodyCubes[i].y = (float)(-(4 - i)) * f3 * 1.7F;
      }
