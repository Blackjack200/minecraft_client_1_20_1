...
public class BatModel extends HierarchicalModel<Bat> {
   ...
   public void setupAnim(Bat bat, float f, float f1, float f2, float f3, float f4) {
      if (bat.isResting()) {
         this.head.xRot = f4 * ((float)Math.PI / 180F);
         this.head.yRot = (float)Math.PI - f3 * ((float)Math.PI / 180F);
         ...
         this.body.xRot = ((float)Math.PI / 4F) + Mth.cos(f2 * 0.1F) * 0.15F;
         this.body.yRot = 0.0F;
         this.rightWing.yRot = Mth.cos(f2 * 74.48451F * ((float)Math.PI / 180F)) * (float)Math.PI * 0.25F;
         this.leftWing.yRot = -this.rightWing.yRot;
