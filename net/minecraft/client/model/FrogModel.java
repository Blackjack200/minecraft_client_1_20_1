...
public class FrogModel<T extends Frog> extends HierarchicalModel<T> {
   ...
   public void setupAnim(T frog, float f, float f1, float f2, float f3, float f4) {
      this.root().getAllParts().forEach(ModelPart::resetPose);
      this.animate(frog.jumpAnimationState, FrogAnimation.FROG_JUMP, f2);
      this.animate(frog.croakAnimationState, FrogAnimation.FROG_CROAK, f2);
      this.animate(frog.tongueAnimationState, FrogAnimation.FROG_TONGUE, f2);
      if (frog.isInWaterOrBubble()) {
         this.animateWalk(FrogAnimation.FROG_SWIM, f, f1, 1.0F, 2.5F);
      } else {
         this.animateWalk(FrogAnimation.FROG_WALK, f, f1, 1.5F, 2.5F);
      }

      this.animate(frog.swimIdleAnimationState, FrogAnimation.FROG_IDLE_WATER, f2);
