...
public class PhantomModel<T extends Phantom> extends HierarchicalModel<T> {
   ...
   public PhantomModel(ModelPart modelpart) {
      ...
      this.tailBase = modelpart1.getChild("tail_base");
      this.tailTip = this.tailBase.getChild("tail_tip");
      this.leftWingBase = modelpart1.getChild("left_wing_base");
      this.leftWingTip = this.leftWingBase.getChild("left_wing_tip");
      this.rightWingBase = modelpart1.getChild("right_wing_base");
   ...
   public void setupAnim(T phantom, float f, float f1, float f2, float f3, float f4) {
      float f5 = ((float)phantom.getUniqueFlapTickOffset() + f2) * 7.448451F * ((float)Math.PI / 180F);
      float f6 = 16.0F;
      this.leftWingBase.zRot = Mth.cos(f5) * 16.0F * ((float)Math.PI / 180F);
      this.leftWingTip.zRot = Mth.cos(f5) * 16.0F * ((float)Math.PI / 180F);
      this.rightWingBase.zRot = -this.leftWingBase.zRot;
      this.rightWingTip.zRot = -this.leftWingTip.zRot;
      this.tailBase.xRot = -(5.0F + Mth.cos(f5 * 2.0F) * 5.0F) * ((float)Math.PI / 180F);
      this.tailTip.xRot = -(5.0F + Mth.cos(f5 * 2.0F) * 5.0F) * ((float)Math.PI / 180F);
   }
}
