...
public class BubbleColumnAmbientSoundHandler implements AmbientSoundHandler {
   ...
   public void tick() {
      ...
      BlockState blockstate = level.getBlockStatesIfLoaded(this.player.getBoundingBox().inflate(0.0D, (double)-0.4F, 0.0D).deflate(1.0E-6D)).filter((blockstate1) -> blockstate1.is(Blocks.BUBBLE_COLUMN)).findFirst().orElse((BlockState)null);
      if (blockstate != null) {
