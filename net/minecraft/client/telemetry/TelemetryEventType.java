...
public class TelemetryEventType {
   ...
   public static final Codec<TelemetryEventType> CODEC = Codec.STRING.comapFlatMap((s) -> {
      TelemetryEventType telemetryeventtype = REGISTRY.get(s);
      return telemetryeventtype != null ? DataResult.success(telemetryeventtype) : DataResult.error(() -> "No TelemetryEventType with key: '" + s + "'");
   ...
   TelemetryEventType(String s, String s1, List<TelemetryProperty<?>> list, boolean flag) {
      this.id = s;
      this.exportKey = s1;
      this.properties = list;
      this.isOptIn = flag;
      this.codec = TelemetryPropertyMap.createCodec(list).xmap((telemetrypropertymap) -> new TelemetryEventInstance(this, telemetrypropertymap), TelemetryEventInstance::properties);
   }

   public static TelemetryEventType.Builder builder(String s, String s1) {
      return new TelemetryEventType.Builder(s, s1);
   }
   ...
   private MutableComponent makeTranslation(String s) {
      return Component.translatable("telemetry.event." + this.id + "." + s);
   }
