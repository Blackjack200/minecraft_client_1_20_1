...
public class GameNarrator {
   ...
   public void sayNow(String s) {
      if (this.getStatus().shouldNarrateSystem() && !s.isEmpty()) {
         this.logNarratedMessage(s);
         if (this.narrator.active()) {
            this.narrator.clear();
            this.narrator.say(s, true);
         }
      }
   ...
   private void logNarratedMessage(String s) {
      if (SharedConstants.IS_RUNNING_IN_IDE) {
         LOGGER.debug("Narrating: {}", (Object)s.replaceAll("\n", "\\\\n"));
      }
   ...
   public void checkStatus(boolean flag) {
      if (flag && !this.isActive() && !TinyFileDialogs.tinyfd_messageBox("Minecraft", "Failed to initialize text-to-speech library. Do you want to continue?\nIf this problem persists, please report it at bugs.mojang.com", "yesno", "error", true)) {
   ...
   public static class NarratorInitException extends SilentInitException {
      public NarratorInitException(String s) {
         super(s);
      }
   }
}
