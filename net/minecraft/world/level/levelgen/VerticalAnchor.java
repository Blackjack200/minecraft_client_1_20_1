...
public interface VerticalAnchor {
   ...
   private static VerticalAnchor merge(Either<VerticalAnchor.Absolute, Either<VerticalAnchor.AboveBottom, VerticalAnchor.BelowTop>> either) {
      return either.map(Function.identity(), (either1) -> either1.map(Function.identity(), Function.identity()));
   }
   ...
   private static Either<VerticalAnchor.Absolute, Either<VerticalAnchor.AboveBottom, VerticalAnchor.BelowTop>> split(VerticalAnchor verticalanchor) {
      return verticalanchor instanceof VerticalAnchor.Absolute ? Either.left((VerticalAnchor.Absolute)verticalanchor) : Either.right(verticalanchor instanceof VerticalAnchor.AboveBottom ? Either.left((VerticalAnchor.AboveBottom)verticalanchor) : Either.right((VerticalAnchor.BelowTop)verticalanchor));
   }
