...
public interface StructureProcessorType<P extends StructureProcessor> {
   ...
   Codec<StructureProcessorList> DIRECT_CODEC = Codec.either(LIST_OBJECT_CODEC.fieldOf("processors").codec(), LIST_OBJECT_CODEC).xmap((either) -> either.map((structureprocessorlist1) -> structureprocessorlist1, (structureprocessorlist) -> structureprocessorlist), Either::left);
   ...
   static <P extends StructureProcessor> StructureProcessorType<P> register(String s, Codec<P> codec) {
      return Registry.register(BuiltInRegistries.STRUCTURE_PROCESSOR, s, () -> codec);
   }
}
