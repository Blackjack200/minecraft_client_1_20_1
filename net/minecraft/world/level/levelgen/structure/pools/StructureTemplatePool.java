...
public class StructureTemplatePool {
   ...
   public static final Codec<StructureTemplatePool> DIRECT_CODEC = RecordCodecBuilder.create((recordcodecbuilder_instance) -> recordcodecbuilder_instance.group(ExtraCodecs.lazyInitializedCodec(CODEC_REFERENCE::getValue).fieldOf("fallback").forGetter(StructureTemplatePool::getFallback), Codec.mapPair(StructurePoolElement.CODEC.fieldOf("element"), Codec.intRange(1, 150).fieldOf("weight")).codec().listOf().fieldOf("elements").forGetter((structuretemplatepool) -> structuretemplatepool.rawTemplates)).apply(recordcodecbuilder_instance, StructureTemplatePool::new));
   ...
   public StructureTemplatePool(Holder<StructureTemplatePool> holder, List<Pair<Function<StructureTemplatePool.Projection, ? extends StructurePoolElement>, Integer>> list, StructureTemplatePool.Projection structuretemplatepool_projection) {
   ...
   public int getMaxSize(StructureTemplateManager structuretemplatemanager) {
      if (this.maxSize == Integer.MIN_VALUE) {
         this.maxSize = this.templates.stream().filter((structurepoolelement1) -> structurepoolelement1 != EmptyPoolElement.INSTANCE).mapToInt((structurepoolelement) -> structurepoolelement.getBoundingBox(structuretemplatemanager, BlockPos.ZERO, Rotation.NONE).getYSpan()).max().orElse(0);
      }
   ...
   public static enum Projection implements StringRepresentable {
      ...
      private Projection(String s, ImmutableList<StructureProcessor> immutablelist) {
         this.name = s;
      ...
      public static StructureTemplatePool.Projection byName(String s) {
