...
public class FlatLevelGeneratorPresets {
   ...
   private static ResourceKey<FlatLevelGeneratorPreset> register(String s) {
      return ResourceKey.create(Registries.FLAT_LEVEL_GENERATOR_PRESET, new ResourceLocation(s));
   }
   ...
   static class Bootstrap {
      ...
      private void register(ResourceKey<FlatLevelGeneratorPreset> resourcekey, ItemLike itemlike, ResourceKey<Biome> resourcekey1, Set<ResourceKey<StructureSet>> set, boolean flag, boolean flag1, FlatLayerInfo... aflatlayerinfo) {
         ...
         HolderSet.Direct<StructureSet> holderset_direct = HolderSet.direct(set.stream().map(holdergetter::getOrThrow).collect(Collectors.toList()));
         FlatLevelGeneratorSettings flatlevelgeneratorsettings = new FlatLevelGeneratorSettings(Optional.of(holderset_direct), holdergetter2.getOrThrow(resourcekey1), FlatLevelGeneratorSettings.createLakesList(holdergetter1));
         if (flag) {
         ...
         for(int i = aflatlayerinfo.length - 1; i >= 0; --i) {
            flatlevelgeneratorsettings.getLayersInfo().add(aflatlayerinfo[i]);
         }
