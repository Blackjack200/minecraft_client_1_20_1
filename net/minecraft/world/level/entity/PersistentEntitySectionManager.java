...
public class PersistentEntitySectionManager<T extends EntityAccess> implements AutoCloseable {
   ...
   private boolean addEntity(T entityaccess, boolean flag) {
      if (!this.addEntityUuid(entityaccess)) {
         ...
         entityaccess.setLevelCallback(new PersistentEntitySectionManager.Callback(entityaccess, i, entitysection));
         if (!flag) {
   ...
   public void updateChunkStatus(ChunkPos chunkpos, Visibility visibility) {
      ...
      this.sectionStorage.getExistingSectionsInChunk(i).forEach((entitysection) -> {
         ...
         if (flag2 && !flag3) {
            entitysection.getEntities().filter((entityaccess3) -> !entityaccess3.isAlwaysTicking()).forEach(this::stopTicking);
         }

         if (flag && !flag1) {
            entitysection.getEntities().filter((entityaccess2) -> !entityaccess2.isAlwaysTicking()).forEach(this::stopTracking);
         } else if (!flag && flag1) {
            entitysection.getEntities().filter((entityaccess1) -> !entityaccess1.isAlwaysTicking()).forEach(this::startTracking);
         }
   ...
   private void ensureChunkQueuedForLoad(long i) {
      PersistentEntitySectionManager.ChunkLoadStatus persistententitysectionmanager_chunkloadstatus = this.chunkLoadStatuses.get(i);
   ...
   private boolean processChunkUnload(long i) {
      boolean flag = this.storeChunkSections(i, (entityaccess) -> entityaccess.getPassengersAndSelf().forEach(this::unloadEntity));
      if (!flag) {
         return false;
      } else {
   ...
   private LongSet getAllChunksToSave() {
      ...
      for(Long2ObjectMap.Entry<PersistentEntitySectionManager.ChunkLoadStatus> long2objectmap_entry : Long2ObjectMaps.fastIterable(this.chunkLoadStatuses)) {
   ...
   class Callback implements EntityInLevelCallback {
      ...
      public void onRemove(Entity.RemovalReason entity_removalreason) {
         if (!this.currentSection.remove(this.entity)) {
            PersistentEntitySectionManager.LOGGER.warn("Entity {} wasn't found in section {} (destroying due to {})", this.entity, SectionPos.of(this.currentSectionKey), entity_removalreason);
         }
