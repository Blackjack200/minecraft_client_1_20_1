...
public class ContentValidationException extends Exception {
   ...
   public static String getMessage(Path path, List<ForbiddenSymlinkInfo> list) {
      return "Failed to validate '" + path + "'. Found forbidden symlinks: " + (String)list.stream().map((forbiddensymlinkinfo) -> forbiddensymlinkinfo.link() + "->" + forbiddensymlinkinfo.target()).collect(Collectors.joining(", "));
   }
}
