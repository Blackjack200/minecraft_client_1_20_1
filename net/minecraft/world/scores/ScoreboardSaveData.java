...
public class ScoreboardSaveData extends SavedData {
   ...
   private void loadTeams(ListTag listtag) {
      for(int i = 0; i < listtag.size(); ++i) {
         ...
         if (compoundtag.contains("NameTagVisibility", 8)) {
            Team.Visibility team_visibility = Team.Visibility.byName(compoundtag.getString("NameTagVisibility"));
         ...
         if (compoundtag.contains("DeathMessageVisibility", 8)) {
            Team.Visibility team_visibility1 = Team.Visibility.byName(compoundtag.getString("DeathMessageVisibility"));
            if (team_visibility1 != null) {
               playerteam.setDeathMessageVisibility(team_visibility1);
            }
         }

         if (compoundtag.contains("CollisionRule", 8)) {
            Team.CollisionRule team_collisionrule = Team.CollisionRule.byName(compoundtag.getString("CollisionRule"));
   ...
   private void loadDisplaySlots(CompoundTag compoundtag) {
      for(int i = 0; i < 19; ++i) {
         if (compoundtag.contains("slot_" + i, 8)) {
            String s = compoundtag.getString("slot_" + i);
   ...
   private void loadObjectives(ListTag listtag) {
      for(int i = 0; i < listtag.size(); ++i) {
         ...
         ObjectiveCriteria.byName(compoundtag.getString("CriteriaName")).ifPresent((objectivecriteria) -> {
            ...
            this.scoreboard.addObjective(s, objectivecriteria, component, objectivecriteria_rendertype);
         });
      }
   ...
   private void saveDisplaySlots(CompoundTag compoundtag) {
      ...
      for(int i = 0; i < 19; ++i) {
         ...
         if (objective != null) {
            compoundtag1.putString("slot_" + i, objective.getName());
            flag = true;
         }
      }

      if (flag) {
         compoundtag.put("DisplaySlots", compoundtag1);
      }
