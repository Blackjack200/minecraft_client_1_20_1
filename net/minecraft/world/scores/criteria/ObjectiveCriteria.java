...
public class ObjectiveCriteria {
   ...
   private static ObjectiveCriteria registerCustom(String s, boolean flag, ObjectiveCriteria.RenderType objectivecriteria_rendertype) {
      ObjectiveCriteria objectivecriteria = new ObjectiveCriteria(s, flag, objectivecriteria_rendertype);
      CUSTOM_CRITERIA.put(s, objectivecriteria);
      return objectivecriteria;
   }

   private static ObjectiveCriteria registerCustom(String s) {
      return registerCustom(s, false, ObjectiveCriteria.RenderType.INTEGER);
   }

   protected ObjectiveCriteria(String s) {
      this(s, false, ObjectiveCriteria.RenderType.INTEGER);
   }

   protected ObjectiveCriteria(String s, boolean flag, ObjectiveCriteria.RenderType objectivecriteria_rendertype) {
      this.name = s;
      this.readOnly = flag;
      this.renderType = objectivecriteria_rendertype;
      CRITERIA_CACHE.put(s, this);
   }
   ...
   public static Optional<ObjectiveCriteria> byName(String s) {
      ObjectiveCriteria objectivecriteria = CRITERIA_CACHE.get(s);
      if (objectivecriteria != null) {
         return Optional.of(objectivecriteria);
      } else {
         int i = s.indexOf(58);
         return i < 0 ? Optional.empty() : BuiltInRegistries.STAT_TYPE.getOptional(ResourceLocation.of(s.substring(0, i), '.')).flatMap((stattype) -> getStat(stattype, ResourceLocation.of(s.substring(i + 1), '.')));
      }
   }
   ...
   public static enum RenderType implements StringRepresentable {
      ...
      public static ObjectiveCriteria.RenderType byId(String s) {
         return CODEC.byName(s, INTEGER);
      }
   }
}
