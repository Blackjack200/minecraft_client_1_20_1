...
public class IndirectMerger implements IndexMerger {
   ...
   public IndirectMerger(DoubleList doublelist, DoubleList doublelist1, boolean flag, boolean flag1) {
      ...
      while(true) {
         ...
         while(true) {
            ...
            flag6 = !flag4 && (flag5 || doublelist.getDouble(i1) < doublelist1.getDouble(j1) + 1.0E-7D);
            if (flag6) {
               ++i1;
               if (!flag2 || j1 != 0 && !flag5) {
                  break;
   ...
   public boolean forMergedIndexes(IndexMerger.IndexConsumer indexmerger_indexconsumer) {
      ...
      for(int j = 0; j < i; ++j) {
         if (!indexmerger_indexconsumer.merge(this.firstIndices[j], this.secondIndices[j], j)) {
            return false;
         }
      }
