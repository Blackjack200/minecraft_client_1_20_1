...
public class CrossbowAttack<E extends Mob & CrossbowAttackMob, T extends LivingEntity> extends Behavior<E> {
   ...
   protected boolean canStillUse(ServerLevel serverlevel, E mob, long i) {
   ...
   protected void stop(ServerLevel serverlevel, E mob, long i) {
      if (mob.isUsingItem()) {
         mob.stopUsingItem();
      }
   ...
   private void crossbowAttack(E mob, LivingEntity livingentity) {
      if (this.crossbowState == CrossbowAttack.CrossbowState.UNCHARGED) {
         ...
         ItemStack itemstack1 = mob.getItemInHand(ProjectileUtil.getWeaponHoldingHand(mob, Items.CROSSBOW));
