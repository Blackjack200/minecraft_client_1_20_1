...
public class GolemRandomStrollInVillageGoal extends RandomStrollGoal {
   ...
   public GolemRandomStrollInVillageGoal(PathfinderMob pathfindermob, double d0) {
      super(pathfindermob, d0, 240, false);
   }
   ...
   private SectionPos getRandomVillageSection() {
      ServerLevel serverlevel = (ServerLevel)this.mob.level();
      List<SectionPos> list = SectionPos.cube(SectionPos.of(this.mob), 2).filter((sectionpos) -> serverlevel.sectionsToVillage(sectionpos) == 0).collect(Collectors.toList());
   ...
   private BlockPos getRandomPoiWithinSection(SectionPos sectionpos) {
      ...
      List<BlockPos> list = poimanager.getInRange((holder) -> true, sectionpos.center(), 8, PoiManager.Occupancy.IS_OCCUPIED).map(PoiRecord::getPos).collect(Collectors.toList());
      return list.isEmpty() ? null : list.get(serverlevel.random.nextInt(list.size()));
   }

   private boolean doesVillagerWantGolem(Villager villager1) {
      return villager1.wantsToSpawnGolem(this.mob.level().getGameTime());
   }
}
