...
public class PathfindToRaidGoal<T extends Raider> extends Goal {
   ...
   private void recruitNearby(Raid raid) {
      if (raid.isActive()) {
         ...
         List<Raider> list = this.mob.level().getEntitiesOfClass(Raider.class, this.mob.getBoundingBox().inflate(16.0D), (raider1) -> !raider1.hasActiveRaid() && Raids.canJoinRaid(raider1, raid));
         set.addAll(list);
