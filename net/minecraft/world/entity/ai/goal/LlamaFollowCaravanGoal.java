...
public class LlamaFollowCaravanGoal extends Goal {
   ...
   public LlamaFollowCaravanGoal(Llama llama, double d0) {
      this.llama = llama;
   ...
   public boolean canUse() {
      if (!this.llama.isLeashed() && !this.llama.inCaravan()) {
         List<Entity> list = this.llama.level().getEntities(this.llama, this.llama.getBoundingBox().inflate(9.0D, 4.0D, 9.0D), (entity2) -> {
   ...
   private boolean firstIsLeashed(Llama llama, int i) {
      if (i > 8) {
         ...
         if (llama.getCaravanHead().isLeashed()) {
            ...
            ++i;
            return this.firstIsLeashed(var10001, i);
         }
      } else {
         return false;
      }
