...
public class MoveThroughVillageGoal extends Goal {
   ...
   public MoveThroughVillageGoal(PathfinderMob pathfindermob, double d0, boolean flag, int i, BooleanSupplier booleansupplier) {
      ...
      if (!GoalUtils.hasGroundPathNavigation(this.mob)) {
         ...
         if (this.onlyAtNight && this.mob.level().isDay()) {
            ...
            if (!serverlevel.isCloseToVillage(blockpos, 6)) {
               ...
               Vec3 vec3 = LandRandomPos.getPos(this.mob, 15, 7, (blockpos5) -> {
                  if (!serverlevel.isVillage(blockpos5)) {
                     return Double.NEGATIVE_INFINITY;
                  } else {
                     Optional<BlockPos> optional1 = serverlevel.getPoiManager().find((holder1) -> holder1.is(PoiTypeTags.VILLAGE), this::hasNotVisited, blockpos5, 10, PoiManager.Occupancy.IS_OCCUPIED);
                     return optional1.map((blockpos7) -> -blockpos7.distSqr(blockpos)).orElse(Double.NEGATIVE_INFINITY);
                  }
               });
               if (vec3 == null) {
                  return false;
   ...
   private boolean hasNotVisited(BlockPos blockpos2) {
      for(BlockPos blockpos3 : this.visited) {
         if (Objects.equals(blockpos2, blockpos3)) {
            return false;
         }
      }
