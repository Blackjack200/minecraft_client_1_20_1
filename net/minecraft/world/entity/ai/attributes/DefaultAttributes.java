...
public class DefaultAttributes {
   ...
   public static void validate() {
      BuiltInRegistries.ENTITY_TYPE.stream().filter((entitytype1) -> entitytype1.getCategory() != MobCategory.MISC).filter((entitytype) -> !hasSupplier(entitytype)).map(BuiltInRegistries.ENTITY_TYPE::getKey).forEach((resourcelocation) -> Util.logAndPauseIfInIde("Entity " + resourcelocation + " has no attributes"));
   }
}
