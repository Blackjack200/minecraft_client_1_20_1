...
public class Schedule {
   ...
   protected static ScheduleBuilder register(String s) {
      Schedule schedule = Registry.register(BuiltInRegistries.SCHEDULE, s, new Schedule());
   ...
   protected List<Timeline> getAllTimelinesExceptFor(Activity activity) {
      return this.timelines.entrySet().stream().filter((map_entry) -> map_entry.getKey() != activity).map(Map.Entry::getValue).collect(Collectors.toList());
   }

   public Activity getActivityAt(int i) {
      return this.timelines.entrySet().stream().max(Comparator.comparingDouble((map_entry) -> (double)map_entry.getValue().getValueAt(i))).map(Map.Entry::getKey).orElse(Activity.IDLE);
   }
}
