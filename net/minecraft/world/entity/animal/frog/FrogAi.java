...
public class FrogAi {
   ...
   private static <E extends Mob> boolean isAcceptableLandingSpot(E mob, BlockPos blockpos) {
      ...
      if (level.getFluidState(blockpos).isEmpty() && level.getFluidState(blockpos1).isEmpty() && level.getFluidState(blockpos.above()).isEmpty()) {
         ...
         if (!blockstate.is(BlockTags.FROG_PREFER_JUMP_TO) && !blockstate1.is(BlockTags.FROG_PREFER_JUMP_TO)) {
            ...
            BlockPathTypes blockpathtypes1 = WalkNodeEvaluator.getBlockPathTypeStatic(level, blockpos1.mutable());
            return blockpathtypes != BlockPathTypes.TRAPDOOR && (!blockstate.isAir() || blockpathtypes1 != BlockPathTypes.TRAPDOOR) ? LongJumpToRandomPos.defaultAcceptableLandingSpot(mob, blockpos) : true;
         } else {
            return true;
         }
      } else {
