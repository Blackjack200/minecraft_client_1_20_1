...
public class Sheep extends Animal implements Shearable {
   ...
   public float getHeadEatAngleScale(float f) {
      if (this.eatAnimationTick > 4 && this.eatAnimationTick <= 36) {
         ...
         return ((float)Math.PI / 5F) + 0.21991149F * Mth.sin(f1 * 28.7F);
      } else {
   ...
   private DyeColor getOffspringColor(Animal animal, Animal animal1) {
      ...
      return this.level().getRecipeManager().getRecipeFor(RecipeType.CRAFTING, craftingcontainer, this.level()).map((craftingrecipe) -> craftingrecipe.assemble(craftingcontainer, this.level().registryAccess())).map(ItemStack::getItem).filter(DyeItem.class::isInstance).map(DyeItem.class::cast).map(DyeItem::getDyeColor).orElseGet(() -> this.level().random.nextBoolean() ? dyecolor : dyecolor1);
   }
   ...
   private static CraftingContainer makeContainer(DyeColor dyecolor, DyeColor dyecolor1) {
      ...
      craftingcontainer.setItem(1, new ItemStack(DyeItem.byColor(dyecolor1)));
