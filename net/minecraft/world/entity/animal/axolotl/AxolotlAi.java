...
public class AxolotlAi {
   ...
   private static void initFightActivity(Brain<Axolotl> brain) {
      brain.addActivityAndRemoveMemoryWhenStopped(Activity.FIGHT, 0, ImmutableList.of(StopAttackingIfTargetInvalid.<Mob>create(Axolotl::onStopAttacking), SetWalkTargetFromAttackTargetIfTargetOutOfReach.create(AxolotlAi::getSpeedModifierChasing), MeleeAttack.create(20), EraseMemoryIf.<Mob>create(BehaviorUtils::isBreeding, MemoryModuleType.ATTACK_TARGET)), MemoryModuleType.ATTACK_TARGET);
   }
   ...
   private static float getSpeedModifierFollowingAdult(LivingEntity livingentity1) {
      return livingentity1.isInWaterOrBubble() ? 0.6F : 0.15F;
   }

   private static float getSpeedModifier(LivingEntity livingentity2) {
      return livingentity2.isInWaterOrBubble() ? 0.5F : 0.15F;
   }
