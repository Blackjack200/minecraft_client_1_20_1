...
public class CamelAi {
   ...
   private static void initIdleActivity(Brain<Camel> brain) {
      brain.addActivity(Activity.IDLE, ImmutableList.of(Pair.of(0, SetEntityLookTargetSometimes.create(EntityType.PLAYER, 6.0F, UniformInt.of(30, 60))), Pair.of(1, new AnimalMakeLove(EntityType.CAMEL, 1.0F)), Pair.of(2, new FollowTemptation((livingentity) -> 2.5F)), Pair.of(3, BehaviorBuilder.triggerIf(Predicate.not(Camel::refuseToMove), BabyFollowAdult.create(ADULT_FOLLOW_RANGE, 2.5F))), Pair.of(4, new RandomLookAround(UniformInt.of(150, 250), 30.0F, 0.0F, 0.0F)), Pair.of(5, new RunOne<>(ImmutableMap.of(MemoryModuleType.WALK_TARGET, MemoryStatus.VALUE_ABSENT), ImmutableList.of(Pair.of(BehaviorBuilder.triggerIf(Predicate.not(Camel::refuseToMove), RandomStroll.stroll(2.0F)), 1), Pair.of(BehaviorBuilder.triggerIf(Predicate.not(Camel::refuseToMove), SetWalkTargetFromLookTarget.create(2.0F, 3)), 1), Pair.of(new CamelAi.RandomSitting(20), 1), Pair.of(new DoNothing(30, 60), 1))))));
   }
   ...
   public static class CamelPanic extends AnimalPanic {
      ...
      protected void start(ServerLevel serverlevel, PathfinderMob pathfindermob, long i) {
         ...
         super.start(serverlevel, pathfindermob, i);
      }
   }
   ...
   public static class RandomSitting extends Behavior<Camel> {
      ...
      protected boolean checkExtraStartConditions(ServerLevel serverlevel, Camel camel) {
         return !camel.isInWater() && camel.getPoseTime() >= (long)this.minimalPoseTicks && !camel.isLeashed() && camel.onGround() && !camel.hasControllingPassenger();
      }

      protected void start(ServerLevel serverlevel, Camel camel, long i) {
