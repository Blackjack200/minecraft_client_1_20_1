...
public class SnifferAi {
   ...
   private static void initIdleActivity(Brain<Sniffer> brain) {
      brain.addActivityWithConditions(Activity.IDLE, ImmutableList.of(Pair.of(0, new AnimalMakeLove(EntityType.SNIFFER, 1.0F) {
         protected void start(ServerLevel serverlevel, Animal animal, long i) {
            ...
            super.start(serverlevel, animal, i);
         }
      }), Pair.of(1, new FollowTemptation((livingentity1) -> 1.25F, (livingentity) -> livingentity.isBaby() ? 2.5D : 3.5D) {
         protected void start(ServerLevel serverlevel, PathfinderMob pathfindermob, long i) {
            SnifferAi.resetSniffing((Sniffer)pathfindermob);
            super.start(serverlevel, pathfindermob, i);
         }
   ...
   static class Digging extends Behavior<Sniffer> {
      ...
      protected boolean canStillUse(ServerLevel serverlevel, Sniffer sniffer, long i) {
         return sniffer.getBrain().getMemory(MemoryModuleType.SNIFFER_DIGGING).isPresent() && sniffer.canDig() && !sniffer.isInLove();
      }

      protected void start(ServerLevel serverlevel, Sniffer sniffer, long i) {
   ...
   static class Searching extends Behavior<Sniffer> {
      ...
      protected boolean canStillUse(ServerLevel serverlevel, Sniffer sniffer, long i) {
         if (!sniffer.canSniff()) {
            ...
            Optional<BlockPos> optional1 = sniffer.getBrain().getMemory(MemoryModuleType.SNIFFER_SNIFFING_TARGET);
