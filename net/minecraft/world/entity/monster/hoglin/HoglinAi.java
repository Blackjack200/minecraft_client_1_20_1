...
public class HoglinAi {
   ...
   private static void initRetreatActivity(Brain<Hoglin> brain) {
      brain.addActivityAndRemoveMemoryWhenStopped(Activity.AVOID, 10, ImmutableList.of(SetWalkTargetAwayFrom.entity(MemoryModuleType.AVOID_TARGET, 1.3F, 15, false), createIdleMovementBehaviors(), SetEntityLookTargetSometimes.create(8.0F, UniformInt.of(30, 60)), EraseMemoryIf.<PathfinderMob>create(HoglinAi::wantsToStopFleeing, MemoryModuleType.AVOID_TARGET)), MemoryModuleType.AVOID_TARGET);
   }
   ...
   private static void broadcastRetreat(Hoglin hoglin, LivingEntity livingentity) {
      getVisibleAdultHoglins(hoglin).forEach((hoglin1) -> retreatFromNearestTarget(hoglin1, livingentity));
   }

   private static void retreatFromNearestTarget(Hoglin hoglin, LivingEntity livingentity) {
      Brain<Hoglin> brain = hoglin.getBrain();
      LivingEntity livingentity1 = BehaviorUtils.getNearestTarget(hoglin, brain.getMemory(MemoryModuleType.AVOID_TARGET), livingentity);
      livingentity1 = BehaviorUtils.getNearestTarget(hoglin, brain.getMemory(MemoryModuleType.ATTACK_TARGET), livingentity1);
      setAvoidTarget(hoglin, livingentity1);
   }
   ...
   static boolean isPosNearNearestRepellent(Hoglin hoglin, BlockPos blockpos) {
      ...
      return optional.isPresent() && optional.get().closerThan(blockpos, 8.0D);
   }
   ...
   private static void maybeRetaliate(Hoglin hoglin, LivingEntity livingentity) {
      if (!hoglin.getBrain().isActive(Activity.AVOID) || livingentity.getType() != EntityType.PIGLIN) {
         if (livingentity.getType() != EntityType.HOGLIN) {
            if (!BehaviorUtils.isOtherTargetMuchFurtherAwayThanCurrentAttackTarget(hoglin, livingentity, 4.0D)) {
   ...
   private static void broadcastAttackTarget(Hoglin hoglin, LivingEntity livingentity) {
      getVisibleAdultHoglins(hoglin).forEach((hoglin1) -> setAttackTargetIfCloserThanCurrent(hoglin1, livingentity));
   }
   ...
   private static void setAttackTargetIfCloserThanCurrent(Hoglin hoglin, LivingEntity livingentity) {
      if (!isPacified(hoglin)) {
         ...
         LivingEntity livingentity1 = BehaviorUtils.getNearestTarget(hoglin, optional, livingentity);
         setAttackTarget(hoglin, livingentity1);
      }
   }
