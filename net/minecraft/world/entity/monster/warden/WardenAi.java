...
public class WardenAi {
   ...
   private static final BehaviorControl<Warden> DIG_COOLDOWN_SETTER = BehaviorBuilder.create((behaviorbuilder_instance) -> behaviorbuilder_instance.<MemoryAccessor>group(behaviorbuilder_instance.registered(MemoryModuleType.DIG_COOLDOWN)).apply(behaviorbuilder_instance, (memoryaccessor) -> (serverlevel, warden, i) -> {
   ...
   protected static Brain<?> makeBrain(Warden warden, Dynamic<?> dynamic) {
      Brain.Provider<Warden> brain_provider = Brain.provider(MEMORY_TYPES, SENSOR_TYPES);
      Brain<Warden> brain = brain_provider.makeBrain(dynamic);
   ...
   private static void initFightActivity(Warden warden, Brain<Warden> brain) {
      brain.addActivityAndRemoveMemoryWhenStopped(Activity.FIGHT, 10, ImmutableList.of(DIG_COOLDOWN_SETTER, StopAttackingIfTargetInvalid.<Warden>create((livingentity2) -> !warden.getAngerLevel().isAngry() || !warden.canTargetEntity(livingentity2), WardenAi::onTargetInvalid, false), SetEntityLookTarget.create((livingentity1) -> isTarget(warden, livingentity1), (float)warden.getAttributeValue(Attributes.FOLLOW_RANGE)), SetWalkTargetFromAttackTargetIfTargetOutOfReach.create(1.2F), new SonicBoom(), MeleeAttack.create(18)), MemoryModuleType.ATTACK_TARGET);
   }
   ...
   private static void onTargetInvalid(Warden warden1, LivingEntity livingentity) {
      if (!warden1.canTargetEntity(livingentity)) {
         warden1.clearAnger(livingentity);
      }

      setDigCooldown(warden1);
   }
