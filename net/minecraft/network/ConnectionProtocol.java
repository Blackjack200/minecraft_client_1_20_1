...
public enum ConnectionProtocol implements BundlerInfo.Provider {
   ...
   private ConnectionProtocol(int i, ConnectionProtocol.ProtocolBuilder connectionprotocol_protocolbuilder) {
      this.id = i;
      this.flows = connectionprotocol_protocolbuilder.flows;
   }

   public int getPacketId(PacketFlow packetflow, Packet<?> packet) {
      return this.flows.get(packetflow).getId(packet.getClass());
   }

   public BundlerInfo getBundlerInfo(PacketFlow packetflow) {
      return this.flows.get(packetflow).bundlerInfo();
   }
   ...
   public Int2ObjectMap<Class<? extends Packet<?>>> getPacketsByIds(PacketFlow packetflow) {
      ...
      ConnectionProtocol.PacketSet<?> connectionprotocol_packetset = this.flows.get(packetflow);
      if (connectionprotocol_packetset == null) {
         return Int2ObjectMaps.emptyMap();
      } else {
         connectionprotocol_packetset.classToId.forEach((oclass, integer) -> int2objectmap.put(integer.intValue(), oclass));
   ...
   public Packet<?> createPacket(PacketFlow packetflow, int i, FriendlyByteBuf friendlybytebuf) {
      return this.flows.get(packetflow).createPacket(i, friendlybytebuf);
   }
   ...
   static {
      for(ConnectionProtocol connectionprotocol : values()) {
         ...
         connectionprotocol.flows.forEach((packetflow, connectionprotocol_packetset) -> connectionprotocol_packetset.listAllPackets((oclass) -> {
               if (PROTOCOL_BY_PACKET.containsKey(oclass) && PROTOCOL_BY_PACKET.get(oclass) != connectionprotocol) {
                  throw new IllegalStateException("Packet " + oclass + " is already assigned to protocol " + PROTOCOL_BY_PACKET.get(oclass) + " - can't reassign to " + connectionprotocol);
               } else {
                  PROTOCOL_BY_PACKET.put(oclass, connectionprotocol);
               }
            }));
      }
   ...
   static class PacketSet<T extends PacketListener> {
      ...
      public <P extends Packet<T>> ConnectionProtocol.PacketSet<T> addPacket(Class<P> oclass, Function<FriendlyByteBuf, P> function) {
      ...
      public <P extends BundlePacket<T>> ConnectionProtocol.PacketSet<T> withBundlePacket(Class<P> oclass, Function<Iterable<Packet<T>>, P> function) {
      ...
      public Packet<?> createPacket(int i, FriendlyByteBuf friendlybytebuf) {
         Function<FriendlyByteBuf, ? extends Packet<T>> function = this.idToDeserializer.get(i);
      ...
      public void listAllPackets(Consumer<Class<? extends Packet<?>>> consumer) {
         this.classToId.keySet().stream().filter((oclass) -> oclass != BundleDelimiterPacket.class).forEach(consumer);
   ...
   static class ProtocolBuilder {
      ...
      public <T extends PacketListener> ConnectionProtocol.ProtocolBuilder addFlow(PacketFlow packetflow, ConnectionProtocol.PacketSet<T> connectionprotocol_packetset) {
         this.flows.put(packetflow, connectionprotocol_packetset);
         return this;
      }
   }
}
