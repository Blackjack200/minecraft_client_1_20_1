   ...
   public void recordingStopped(@Nullable Path path) {
      if (path != null) {
         ...
         try {
            ...
            Path path1 = path.resolveSibling("jfr-report-" + StringUtils.substringBefore(path.getFileName().toString(), ".jfr") + ".json");
            Files.writeString(path1, jfrstatsresult.asJson(), StandardOpenOption.CREATE);
            infoWithFallback(() -> "Dumped recording summary to " + path1);
