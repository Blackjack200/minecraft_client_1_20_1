...
public class MetricSampler {
   ...
   protected MetricSampler(String s, MetricCategory metriccategory, DoubleSupplier doublesupplier, @Nullable Runnable runnable, @Nullable MetricSampler.ThresholdTest metricsampler_thresholdtest) {
      this.name = s;
      this.category = metriccategory;
      this.beforeTick = runnable;
      this.sampler = doublesupplier;
      this.thresholdTest = metricsampler_thresholdtest;
   ...
   public static MetricSampler create(String s, MetricCategory metriccategory, DoubleSupplier doublesupplier) {
      return new MetricSampler(s, metriccategory, doublesupplier, (Runnable)null, (MetricSampler.ThresholdTest)null);
   }

   public static <T> MetricSampler create(String s, MetricCategory metriccategory, T object, ToDoubleFunction<T> todoublefunction) {
      return builder(s, metriccategory, todoublefunction, object).build();
   }

   public static <T> MetricSampler.MetricSamplerBuilder<T> builder(String s, MetricCategory metriccategory, ToDoubleFunction<T> todoublefunction, T object) {
      return new MetricSampler.MetricSamplerBuilder<>(s, metriccategory, todoublefunction, object);
   }
   ...
   public static class MetricSamplerBuilder<T> {
      ...
      public MetricSamplerBuilder(String s, MetricCategory metriccategory, ToDoubleFunction<T> todoublefunction, T object) {
         this.name = s;
         this.category = metriccategory;
      ...
      public MetricSampler.MetricSamplerBuilder<T> withThresholdAlert(MetricSampler.ThresholdTest metricsampler_thresholdtest) {
         this.thresholdTest = metricsampler_thresholdtest;
         return this;
      }
   ...
   public static class ValueIncreasedByPercentage implements MetricSampler.ThresholdTest {
      ...
      public boolean test(double d0) {
         ...
         if (this.previousValue != Double.MIN_VALUE && !(d0 <= this.previousValue)) {
            flag1 = (d0 - this.previousValue) / this.previousValue >= (double)this.percentageIncreaseThreshold;
         } else {
            flag1 = false;
         }
