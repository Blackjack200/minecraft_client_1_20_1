...
public class ThreadingDetector {
   ...
   public static ReportedException makeThreadingException(String s, @Nullable Thread thread) {
      String s1 = Stream.of(Thread.currentThread(), thread).filter(Objects::nonNull).map(ThreadingDetector::stackTrace).collect(Collectors.joining("\n"));
   ...
   private static String stackTrace(Thread thread1) {
      return thread1.getName() + ": \n\tat " + (String)Arrays.stream(thread1.getStackTrace()).map(Object::toString).collect(Collectors.joining("\n\tat "));
   }
}
