...
public abstract class FloatProvider implements SampledFloat {
   ...
   public static final Codec<FloatProvider> CODEC = CONSTANT_OR_DISPATCH_CODEC.xmap((either) -> either.map(ConstantFloat::of, (floatprovider) -> floatprovider), (floatprovider) -> floatprovider.getType() == FloatProviderType.CONSTANT ? Either.left(((ConstantFloat)floatprovider).getValue()) : Either.right(floatprovider));
   ...
   public static Codec<FloatProvider> codec(float f, float f1) {
      return ExtraCodecs.validate(CODEC, (floatprovider) -> {
         if (floatprovider.getMinValue() < f) {
            ...
            return floatprovider.getMaxValue() > f1 ? DataResult.error(() -> "Value provider too high: " + f1 + " [" + floatprovider.getMinValue() + "-" + floatprovider.getMaxValue() + "]") : DataResult.success(floatprovider);
         }
      });
   }
