...
public class Mth {
   ...
   private static final float[] SIN = Util.make(new float[65536], (afloat) -> {
   ...
   public static double clampedLerp(double d0, double d1, double d2) {
      if (d2 < 0.0D) {
         ...
         return d2 > 1.0D ? d1 : lerp(d2, d0, d1);
      }
   }
   ...
   public static float clampedLerp(float f, float f1, float f2) {
      if (f2 < 0.0F) {
         ...
         return f2 > 1.0F ? f1 : lerp(f2, f, f1);
      }
   }
   ...
   public static boolean equal(double d0, double d1) {
      return Math.abs(d1 - d0) < (double)1.0E-5F;
   }
   ...
   public static int color(float f, float f1, float f2) {
      return FastColor.ARGB32.color(0, floor(f * 255.0F), floor(f1 * 255.0F), floor(f2 * 255.0F));
   }
   ...
   public static boolean rayIntersectsAABB(Vec3 vec3, Vec3 vec31, AABB aabb) {
      ...
      double d2 = vec3.x - d0;
      if (Math.abs(d2) > d1 && d2 * vec31.x >= 0.0D) {
         return false;
      } else {
         double d3 = (aabb.minY + aabb.maxY) * 0.5D;
         double d4 = (aabb.maxY - aabb.minY) * 0.5D;
         double d5 = vec3.y - d3;
         if (Math.abs(d5) > d4 && d5 * vec31.y >= 0.0D) {
            return false;
         } else {
            double d6 = (aabb.minZ + aabb.maxZ) * 0.5D;
            double d7 = (aabb.maxZ - aabb.minZ) * 0.5D;
            double d8 = vec3.z - d6;
            if (Math.abs(d8) > d7 && d8 * vec31.z >= 0.0D) {
               return false;
            } else {
               double d9 = Math.abs(vec31.x);
               double d10 = Math.abs(vec31.y);
               double d11 = Math.abs(vec31.z);
               double d12 = vec31.y * d8 - vec31.z * d5;
               if (Math.abs(d12) > d4 * d11 + d7 * d10) {
                  return false;
               } else {
                  d12 = vec31.z * d2 - vec31.x * d8;
                  if (Math.abs(d12) > d1 * d11 + d7 * d9) {
                     return false;
                  } else {
                     d12 = vec31.x * d5 - vec31.y * d2;
   ...
   public static double atan2(double d0, double d1) {
      ...
      if (Double.isNaN(d2)) {
         ...
         d1 *= d4;
         d0 *= d4;
         double d5 = FRAC_BIAS + d0;
         int i = (int)Double.doubleToRawLongBits(d5);
         double d6 = ASIN_TAB[i];
         double d7 = COS_TAB[i];
         double d8 = d5 - FRAC_BIAS;
         double d9 = d0 * d7 - d1 * d8;
   ...
   public static int hsvToRgb(float f, float f1, float f2) {
      ...
      return FastColor.ARGB32.color(0, clamp((int)(f7 * 255.0F), 0, 255), clamp((int)(f8 * 255.0F), 0, 255), clamp((int)(f9 * 255.0F), 0, 255));
   }
   ...
   public static IntStream outFromOrigin(int i, int j, int k, int l) {
      if (j > k) {
         ...
         return i >= j && i <= k ? IntStream.iterate(i, (k3) -> {
            int l3 = Math.abs(i - k3);
            return i - l3 >= j || i + l3 <= k;
         }, (i2) -> {
            boolean flag = i2 <= i;
            int j2 = Math.abs(i - i2);
            boolean flag1 = i + j2 + l <= k;
            if (!flag || !flag1) {
               int k2 = i - j2 - (flag ? l : 0);
               if (k2 >= j) {
                  return k2;
               }
            }
