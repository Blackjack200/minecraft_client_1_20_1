...
public class StatsCounterFix extends DataFix {
   ...
   public TypeRewriteRule makeRule() {
      ...
      return this.fixTypeEverywhereTyped("StatsCounterFix", this.getInputSchema().getType(References.STATS), type, (typed) -> {
         ...
         if (optional.isPresent()) {
            ...
            while(true) {
               Map.Entry<? extends Dynamic<?>, ? extends Dynamic<?>> map_entry;
               String s1;
               String s2;
               while(true) {
                  if (!var6.hasNext()) {
                  ...
                  map_entry = (Map.Entry)var6.next();
                  if (map_entry.getValue().asNumber().result().isPresent()) {
                     String s = map_entry.getKey().asString("");
                     if (!SKIP.contains(s)) {
                        ...
                        if (i >= 0) {
                           ...
                           if (ITEM_KEYS.containsKey(s3)) {
                              s1 = ITEM_KEYS.get(s3);
                              String s7 = s.substring(i + 1).replace('.', ':');
                              String s8 = this.upgradeItem(s7);
                              s2 = s8 == null ? s7 : s8;
                              break;
                           }
                           ...
                           if (ENTITY_KEYS.containsKey(s3)) {
                              ...
                              String s11 = s.substring(i + 1).replace('.', ':');
                              s2 = ENTITIES.getOrDefault(s11, s11);
                              break;
                           }
                        }
                     }
               ...
               Dynamic<?> dynamic2 = map.computeIfAbsent(dynamic1, (dynamic4) -> dynamic.emptyMap());
               map.put(dynamic1, dynamic2.set(s2, map_entry.getValue()));
            }
         } else {
            return type.readTyped(dynamic.emptyMap().set("stats", dynamic.createMap(map))).result().orElseThrow(() -> new IllegalStateException("Could not parse new stats object.")).getFirst();
         }
   ...
   protected String upgradeItem(String s) {
      return ItemStackTheFlatteningFix.updateItem(s, 0);
   }
