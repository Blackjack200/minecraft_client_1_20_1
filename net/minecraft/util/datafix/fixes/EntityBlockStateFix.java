...
public class EntityBlockStateFix extends DataFix {
   ...
   public EntityBlockStateFix(Schema schema, boolean flag) {
      super(schema, flag);
   }
   ...
   public TypeRewriteRule makeRule() {
      ...
      Function<Typed<?>, Typed<?>> function = (typed4) -> this.updateBlockToBlockState(typed4, "DisplayTile", "DisplayData", "DisplayState");
      Function<Typed<?>, Typed<?>> function1 = (typed3) -> this.updateBlockToBlockState(typed3, "inTile", "inData", "inBlockState");
      ...
      Function<Typed<?>, Typed<?>> function2 = (typed2) -> typed2.update(type.finder(), DSL.remainderType(), Pair::getSecond);
      return this.fixTypeEverywhereTyped("EntityBlockStateFix", schema.getType(References.ENTITY), schema1.getType(References.ENTITY), (typed) -> {
         ...
         typed = this.updateEntity(typed, "minecraft:enderman", (typed1) -> this.updateBlockToBlockState(typed1, "carried", "carriedData", "carriedBlockState"));
   ...
   private Typed<?> updateFallingBlock(Typed<?> typed) {
      ...
      return typed.update(type.finder(), type1, (either) -> {
         int i = either.map((pair) -> pair.getSecond().map((integer) -> integer, EntityBlockStateFix::getBlockId), (unit) -> {
            Optional<Number> optional = dynamic.get("TileID").asNumber().result();
            return optional.map(Number::intValue).orElseGet(() -> dynamic.get("Tile").asByte((byte)0) & 255);
         });
   ...
   private Typed<?> updateBlockToBlockState(Typed<?> typed, String s, String s1, String s2) {
      Type<Pair<String, Either<Integer, String>>> type = DSL.field(s, DSL.named(References.BLOCK_NAME.typeName(), DSL.or(DSL.intType(), NamespacedSchema.namespacedString())));
      Type<Pair<String, Dynamic<?>>> type1 = DSL.field(s2, DSL.named(References.BLOCK_STATE.typeName(), DSL.remainderType()));
   ...
   private Typed<?> updateEntity(Typed<?> typed, String s, Function<Typed<?>, Typed<?>> function) {
