...
public class OptionsKeyLwjgl3Fix extends DataFix {
   ...
   public OptionsKeyLwjgl3Fix(Schema schema, boolean flag) {
      super(schema, flag);
   }

   public TypeRewriteRule makeRule() {
      return this.fixTypeEverywhereTyped("OptionsKeyLwjgl3Fix", this.getInputSchema().getType(References.OPTIONS), (typed) -> typed.update(DSL.remainderFinder(), (dynamic) -> dynamic.getMapValues().map((map) -> dynamic.createMap(map.entrySet().stream().map((map_entry) -> {
                  if (map_entry.getKey().asString("").startsWith("key_")) {
                     int i = Integer.parseInt(map_entry.getValue().asString(""));
                     if (i < 0) {
                        int j = i + 100;
                        String s;
                        if (j == 0) {
                        ...
                        return Pair.of(map_entry.getKey(), map_entry.getValue().createString(s));
                     } else {
                        String s4 = MAP.getOrDefault(i, "key.unknown");
                        return Pair.of(map_entry.getKey(), map_entry.getValue().createString(s4));
                     }
                  } else {
