...
public class EntityHorseSaddleFix extends NamedEntityFix {
   public EntityHorseSaddleFix(Schema schema, boolean flag) {
      super(schema, flag, "EntityHorseSaddleFix", References.ENTITY, "EntityHorse");
   }
   ...
   protected Typed<?> fix(Typed<?> typed) {
      ...
      if (!optional.isPresent() && dynamic.get("Saddle").asBoolean(false)) {
         Typed<?> typed1 = type.pointTyped(typed.getOps()).orElseThrow(IllegalStateException::new);
