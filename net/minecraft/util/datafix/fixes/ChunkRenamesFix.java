...
public class ChunkRenamesFix extends DataFix {
   ...
   private static Typed<?> renameField(Typed<?> typed, String s, String s1) {
      return renameFieldHelper(typed, s, s1, typed.getType().findFieldType(s)).update(DSL.remainderFinder(), (dynamic) -> dynamic.remove(s));
   }
   ...
   private static <T> Dynamic<T> mergeRemainders(Typed<?> typed, Dynamic<T> dynamic) {
      ...
      Dynamic<T> dynamic1 = typed.get(DSL.remainderFinder()).convert(dynamicops);
      DataResult<T> dataresult = dynamicops.getMap(dynamic.getValue()).flatMap((maplike) -> dynamicops.mergeToMap(dynamic1.getValue(), maplike));
