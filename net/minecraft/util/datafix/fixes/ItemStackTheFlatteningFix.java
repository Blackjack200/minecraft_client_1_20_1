...
public class ItemStackTheFlatteningFix extends DataFix {
   ...
   private static final Set<String> IDS = MAP.keySet().stream().map((s) -> s.substring(0, s.indexOf(46))).collect(Collectors.toSet());
   ...
   public static String updateItem(@Nullable String s, int i) {
      if (IDS.contains(s)) {
         String s1 = MAP.get(s + "." + i);
         return s1 == null ? MAP.get(s + ".0") : s1;
      } else {
         return null;
      }
   }
