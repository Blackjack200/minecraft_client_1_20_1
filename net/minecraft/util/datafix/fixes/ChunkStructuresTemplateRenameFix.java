...
public class ChunkStructuresTemplateRenameFix extends DataFix {
   ...
   private Dynamic<?> fixChildren(Dynamic<?> dynamic) {
      return dynamic.update("Children", (dynamic2) -> dynamic.createList(dynamic2.asStream().map((dynamic4) -> this.fixTag(dynamic, dynamic4))));
   }

   private Dynamic<?> fixTag(Dynamic<?> dynamic, Dynamic<?> dynamic1) {
      String s = dynamic.get("id").asString("");
      if (RENAMES.containsKey(s)) {
         Pair<String, ImmutableMap<String, String>> pair = RENAMES.get(s);
         if (pair.getFirst().equals(dynamic1.get("id").asString(""))) {
            String s1 = dynamic1.get("Template").asString("");
            dynamic1 = dynamic1.set("Template", dynamic1.createString(pair.getSecond().getOrDefault(s1, s1)));
         }
      }
