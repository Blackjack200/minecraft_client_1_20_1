...
public class GsonHelper {
   ...
   public static Item convertToItem(JsonElement jsonelement, String s) {
      if (jsonelement.isJsonPrimitive()) {
         ...
         return BuiltInRegistries.ITEM.getOptional(new ResourceLocation(s1)).orElseThrow(() -> new JsonSyntaxException("Expected " + s + " to be an item, was unknown string '" + s1 + "'"));
      } else {
   ...
   public static <T> T getAsObject(JsonObject jsonobject, String s, @Nullable T object, JsonDeserializationContext jsondeserializationcontext, Class<? extends T> oclass) {
      return (T)(jsonobject.has(s) ? convertToObject(jsonobject.get(s), s, jsondeserializationcontext, oclass) : object);
   }
   ...
   public static <T> T fromJson(Gson gson, Reader reader, Class<T> oclass, boolean flag) {
      T object = fromNullableJson(gson, reader, oclass, flag);
      if (object == null) {
   ...
   public static <T> T fromJson(Gson gson, Reader reader, TypeToken<T> typetoken, boolean flag) {
      T object = fromNullableJson(gson, reader, typetoken, flag);
      if (object == null) {
         throw new JsonParseException("JSON data was null or empty");
      } else {
         return object;
   ...
   public static <T> T fromNullableJson(Gson gson, String s, TypeToken<T> typetoken, boolean flag) {
      return fromNullableJson(gson, new StringReader(s), typetoken, flag);
   }

   public static <T> T fromJson(Gson gson, String s, Class<T> oclass, boolean flag) {
      return fromJson(gson, new StringReader(s), oclass, flag);
   }

   @Nullable
   public static <T> T fromNullableJson(Gson gson, String s, Class<T> oclass, boolean flag) {
      return fromNullableJson(gson, new StringReader(s), oclass, flag);
   }
   ...
   public static <T> T fromNullableJson(Gson gson, String s, TypeToken<T> typetoken) {
      return fromNullableJson(gson, s, typetoken, false);
   }
   ...
   public static JsonObject parse(String s, boolean flag) {
      return parse(new StringReader(s), flag);
   }

   public static JsonObject parse(Reader reader, boolean flag) {
      return fromJson(GSON, reader, JsonObject.class, flag);
   }
      ...
      if (jsonelement != null && !jsonelement.isJsonNull()) {
         if (jsonelement.isJsonPrimitive()) {
            ...
            for(Map.Entry<String, JsonElement> map_entry : sortByKeyIfNeeded(jsonelement.getAsJsonObject().entrySet(), comparator)) {
               jsonwriter.name(map_entry.getKey());
               writeValue(jsonwriter, map_entry.getValue(), comparator);
            }
